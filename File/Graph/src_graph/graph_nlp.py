...
class Token:
    ...
    def remove_connect(self):
        unique_con_index = [index for index in self.p_con_index if index not in self.p_index]
        self.p_con_index = unique_con_index
...
def create_list_clusters_token(doc, sent):
    """
    Функция создает список кластеров токенов на основе предложения.
...
def create_list_clusters_entity(doc, sent):
    """
    Функция создает кластеры именованных сущностей на основе предложения.
...
def remove_subsets_clusters(clusters):
    """
    Функция удаляет подмножества кластеров из списка кластеров.
...
def resolve_connect(doc, sent):
    """
    Функция разрешает равнозначные связи в предложении, сохраняя структуру дерева зависимостей.
...
def create_list_clusters(doc, sent):
    """
    Функция создает кластеры токенов на основе предложения, используя предварительно определенные кластеры сущностей и
    кластеры токенов.
    ...
    clusters_entity = create_list_clusters_entity(doc, sent)
    clusters_token  = create_list_clusters_token(doc, sent)
